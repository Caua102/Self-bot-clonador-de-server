...
(async function () {
	setTitle("Cauã Clone | Inicio");
	console.log(" ");
	console.log(" ");
	console.log(center(`  ::::::::      :::     :::    :::     :::      ::::::::   ::::::::  :::::::::  ::::::::::: :::::::::  :::::::::::  `.red, 112));
	console.log(center(` :+:    :+:   :+: :+:   :+:    :+:   :+: :+:   :+:    :+: :+:    :+: :+:    :+:     :+:     :+:    :+:     :+:      `.red, 112));
	console.log(center(` +:+         +:+   +:+  +:+    +:+  +:+   +:+  +:+        +:+        +:+    +:+     +:+     +:+    +:+     +:+      `.red, 112));
	console.log(center(` +#+        +#++:++#++: +#+    +:+ +#++:++#++: +#++:++#++ +#+        +#++:++#:      +#+     +#++:++#+      +#+      `.red, 112));
	console.log(center(` +#+        +#+     +#+ +#+    +#+ +#+     +#+        +#+ +#+        +#+    +#+     +#+     +#+            +#+      `.red, 112));
...
console.log(" ");
console.log("------------------------------------------------------------------------------------------------------------------")
let token = await question(('- Coloque o Token da sua conta do discord bb:'.yellow));
console.log("------------------------------------------------------------------------------------------------------------------")
let copy = await question(('- Coloque o Id do servidor que você quer copiar bb:'.yellow));
console.log("------------------------------------------------------------------------------------------------------------------")
let paste = await question(('- Coloque o Id do servidor que você irá colar bb::'.yellow));
console.log("------------------------------------------------------------------------------------------------------------------")
...
client.on("ready", async() => {
	try {
		...
		log.info(`Entrando na conta ${client.user.tag} ( ${client.user.email})`)
	} catch (e) {
		log.error(e.stack)
	}


	setTitle("Cauã fofo | Clonando Server");
  ...
  let region = allowedRegions.includes(guild2.region) ? guild2.region : 'eu-central';
	...
	let guildname = guild1.name + " - Cauã";
  ...
  await guild2.setVerificationLevel(guild1.verificationLevel).then(m => {
	...
	log.info('Servidor Clonado por Cauã.')
})
